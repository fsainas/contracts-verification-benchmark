...
rule validate_transfer_same_s_r {
   env e;
   ...
   assert(balanceOf(owner) == oldOwnerBalance + expectedFee);
   assert(balanceOf(e.msg.sender) == oldSenderBalance - expectedFee);
   assert(balanceOf(receiver) == oldReceiverBalance - expectedFee);
}
